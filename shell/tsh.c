...
eval(char *cmdline) 
{
    ...
    if (tok.builtins == BUILTIN_JOBS){
        listjobs(job_list, getout_fd(tok.outfile));
        return;
    }
    /* FG */
    if (tok.builtins == BUILTIN_FG){
        if (!(child_pid = parse_pid(tok.argv[1])))
            printf("provide proper numbers: (%%jid) or (pid)\n");
        else{
            getjobpid(job_list, child_pid)->state = FG;
            /* send SIGCONT */
            kill(child_pid, SIGCONT);
            
            /* block until fg job finish */
    ...
    if (tok.builtins == BUILTIN_BG){
        ...
        printf("[%d] (%d) %s\n", pid2jid(child_pid), child_pid,
                getjobpid(job_list, child_pid)->cmdline);
        getjobpid(job_list, child_pid)->state = BG;
        /* send SIGCONT */
        kill(child_pid, SIGCONT);
        return;
    }
    ...
    /* block certain signals to avoid race conditions */
    sigemptyset(&sset);
    ...
    if (child_pid){
        if (!addjob(job_list, child_pid, bg? BG: FG, cmdline))
            /* addjob fails for too many jobs */
            exit(1);
        if (bg)
            printf("[%d] (%d) %s\n", pid2jid(child_pid), child_pid, cmdline);
        else{
            /* block until fg job finish */
            while (fgpid(job_list))
                sigsuspend(&oset);
    ...
    /* child */
        ...
        /* redirection */
        dup2(getout_fd(tok.outfile), STDOUT_FILENO);
        dup2(getin_fd(tok.infile), STDIN_FILENO);
...
/*
 * getout_fd - open file for output.
 *
 *
 */
int
getout_fd(char* filename){
    int ret;
    if (!filename)
        return STDOUT_FILENO;
    ret = open(filename, O_WRONLY | O_CREAT | O_TRUNC,
...
/*
 * getin_fd - open file for input.
 *
 *
 */
int
getin_fd(char* filename){
    int ret;
    if (!filename)
        return STDIN_FILENO;
...
/*
 * parse_pid - parse argument of fg/bg
 */
pid_t
parse_pid(char *arg){
    if (*arg == '%')
        return getjobjid(job_list, atoi(arg + 1))->pid;
 ...
 *     a child job terminates (becomes a zoqmbie), or stops because it
