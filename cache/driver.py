...
def main():
	...
	trans32_score = computeMissScore(miss32, 300, 600, maxscore['trans32']) * trans_cscore
	trans64_score = computeMissScore(miss64, 1300, 2000, maxscore['trans64']) * trans_cscore
	trans61_score = computeMissScore(miss61, 2000, 3000, maxscore['trans61']) * trans_cscore
	total_score = csim_cscore[0] + trans_cscore + trans32_score + trans64_score + trans61_score

	# Summarize the results
	print "\nCache Lab summary:"
	print "%-22s%8s%10s%8s" % ("", "Points", "Max pts", "Misses")
	print "%-22s%8.1f%10d" % ("Csim correctness", csim_cscore[0], 
							   maxscore['csim'])
	print "%-22s%8.1f%10d" % ("Trans correctness", trans_cscore,
							   maxscore['transc'])
	print "%-22s%8.1f%10d%8d" % ("Trans perf 32x32", trans32_score, 
								  maxscore['trans32'], miss32)
	print "%-22s%8.1f%10d%8d" % ("Trans perf 64x64", trans64_score, 
							 maxscore['trans64'], miss64)
	print "%-22s%8.1f%10d%8d" % ("Trans perf 61x67", trans61_score, 
							 maxscore['trans61'], miss61)
	print "%22s%8.1f%10d" % ("Total points", total_score,
							  maxscore['csim'] + maxscore['transc'] + 
							  maxscore['trans32'] + maxscore['trans64'] +
							  maxscore['trans61'])
	
	# Emit autoresult string for Autolab
	if autograde:
