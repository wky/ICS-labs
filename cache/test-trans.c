...
static struct results results = {-1, -1, -1};

/*
 * eval_correctness - Check each of the registered transpose functions
 * for correctness
 */
void eval_correctness(int A[N][M], int B[M][N], int C[M][N])
{
	int i, j, k;

	printf("Step 1: Evaluating registered transpose funcs for correctness:\n");
	...
	/* Compute the correct transpose of A */
	correctTrans(M, N, A, C); 

	/* Iterate over each registered function */
	for (i=0; i < func_counter; i++) {
		/* Before every test, A and B need to be restored because: (1)
		 * the student might have modified A, (2) The student could
		 * use whatever is in B and return it directly and thus have
		...
		for (j=0; j<M; j++) {
			for (k=0; k<N; k++) {
				if (B[j][k] != C[j][k]) {
					func_list[i].correct = 0;
					break;
				}
			}
		}
		printf("func %d (%s): correctness: %u\n", 
			   i, func_list[i].description, func_list[i].correct);
		...
		if (strcmp(func_list[i].description, SUBMIT_DESCRIPTION) == 0 ) {
			...
			results.correct = func_list[i].correct;
		}
	}
}
...
void eval_perf(unsigned int s, unsigned int E, unsigned int b)
{
	...
	/* Use valgrind to generate traces of all of the registered trace
	   functions in a single trace file */
	sprintf(cmd, "valgrind --tool=lackey --trace-mem=yes --log-fd=1 -v ./tracegen -M %d -N %d  > trace.all", M, N);
	system(cmd);
	...
	FILE* full_trace_fp;  
	FILE* part_trace_fp; 
	full_trace_fp = fopen("trace.all", "r");
	assert(full_trace_fp);


	/* Evaluate the performance of each registered transpose function */
	printf("\nStep 3: Evaluating performance of registered transpose funcs (s=%d, E=%d, b=%d)\n", s, E, b);
	for (i=0; i<func_counter; i++) {
		...
		while (fgets(buf, 1000, full_trace_fp) != NULL) {
			...
			/* We are only intested in memory access instructions */
			if (buf[0]==' ' && buf[2]==' ' &&
...
int main(int argc, char* argv[])
{
	...
	/* Check the student's transpose function for correctness */
	eval_correctness(A, B, C);
